...
def criar_cliente(cliente: Cliente, user: dict = Depends(get_current_user)):
    if user['role'] not in ['AnalistadeCadastro', 'admin']:
        raise HTTPException(
            status_code=403, detail='Apenas gerentes podem cadastrar clientes'
        )
...
def atualizar_cliente(
    id: str, cliente: ClienteUpdate, user: dict = Depends(get_current_user)
):

    if user['role'] not in ['AnalistadeCadastro', 'admin']:
        raise HTTPException(
            status_code=403, detail='Apenas gerentes podem atualizar clientes'
        )
    ...
    update_data = {
        key: value for key, value in cliente.dict(exclude_unset=True).items()
    }
...
def deletar_cliente(id: str, user: dict = Depends(get_current_user)):
    if user['role'] not in ['AnalistadeCadastro', 'admin']:
        raise HTTPException(
            status_code=403,
            detail='Apenas o Analista de Cadastro pode excluir clientes',
        )
