...
@router.post('/')
def criar_pagamento(
    pagamento: Pagamento, user: dict = Depends(get_current_user)
):
    if user['role'] not in ['Caixa', 'admin']:
        raise HTTPException(
            status_code=403, detail='Apenas caixas podem registrar pagamentos'
        )
    ...
    cliente = db.clientes.find_one(
        {'cpf': locacao['cpf']}, {'_id': 0, 'nome': 1, 'cpf': 1}
    )
    if not cliente:
