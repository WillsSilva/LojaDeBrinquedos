...
@router.post('/')
def criar_brinquedo(
    brinquedo: Brinquedo, user: dict = Depends(get_current_user)
):
    if user['role'] not in ['Almoxarife', 'admin']:
        raise HTTPException(
            status_code=403,
            detail='Apenas almoxarifes podem cadastrar brinquedos',
        )
...
@router.put('/{id}')
def atualizar_brinquedo(
    id: str, brinquedo: Brinquedo, user: dict = Depends(get_current_user)
):
    if user['role'] not in ['Almoxarife', 'admin']:
        raise HTTPException(
            status_code=403,
            detail='Apenas almoxarifes podem atualizar brinquedos',
        )
...
def obter_brinquedo(id: str, user: dict = Depends(get_current_user)):
    brinquedo = db.brinquedos.find_one(
        {'ID': int(id)}, {'_id': 0}
    )  # Não retorna o _id
    if not brinquedo:
        raise HTTPException(status_code=404, detail='Brinquedo não encontrado')
