...
def criar_locacao(locacao: Locacao, user: dict = Depends(get_current_user)):
    if user["role"] not in ["AgenteDeLocacao", "admin"]:    
        raise HTTPException(status_code=403, detail="Apenas agentes de locação podem registrar locações")

    if not locacao.brinquedos:
        raise HTTPException(status_code=400, detail="É necessário ao menos um brinquedo na locação")
...
def deletar_locacao(id: str, user: dict = Depends(get_current_user)):
    if user["role"] not in ["AgenteDeLocacao", "admin"]:
        raise HTTPException(status_code=403, detail="Apenas agentes de locação podem deletar locações")
