...
from models.schemas import Funcionario, FuncionarioResponse, FuncionarioUpdate
...
def criar_funcionario(
    funcionario: Funcionario, user: dict = Depends(get_current_user)
):
    if user['role'] not in ['gerente', 'admin']:
        raise HTTPException(
            status_code=403,
            detail='Apenas gerentes podem cadastrar funcionários',
        )

    # Verificar se já existe um usuário com o mesmo username
    if db.funcionarios.find_one({'username': funcionario.username}):
        raise HTTPException(
            status_code=400, detail='Nome de usuário já está em uso'
        )
...
def atualizar_funcionario(
    ...
    if user['role'] not in ['gerente', 'admin']:
        raise HTTPException(
            status_code=403,
            detail='Apenas gerentes podem atualizar funcionários',
        )
    ...
    if funcionario.username:
        username_exists = db.funcionarios.find_one(
            {'username': funcionario.username}
        )
        if (
            username_exists
            and username_exists['_id'] != existing_funcionario['_id']
        ):
            raise HTTPException(
                status_code=400, detail='Nome de usuário já está em uso'
            )
...
def deletar_funcionario(id: str, user: dict = Depends(get_current_user)):
    ...
    if user['role'] not in ['gerente', 'admin']:
        raise HTTPException(
            status_code=403,
            detail='Apenas gerentes podem excluir funcionários',
        )
